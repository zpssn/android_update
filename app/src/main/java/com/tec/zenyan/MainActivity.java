...
//                startActivity(intent);
    ...
    private void set_dbimage(){
        ...
        if(bitmap_result==null){
            ...
            Log.i(TAG,"null:"+bytesToHexString(byteArray));
            Log.i(TAG,"null:"+hexStringToBytes(bytesToHexString(byteArray)));
            byte[] out = hexStringToBytes(bytesToHexString(byteArray));
            Bitmap bmp = BitmapFactory.decodeByteArray(out, 0, out.length);
            welcome_image.setImageBitmap(bmp);
